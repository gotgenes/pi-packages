...
`isSubagentExecutionContext` is unchanged and still answers `true` for such a root — it means "is this process a child", not "should this node relay" — so do not re-add the consult, and do not read that predicate anywhere without testing `hasUI` first, as `selectAuthorizer` already does.

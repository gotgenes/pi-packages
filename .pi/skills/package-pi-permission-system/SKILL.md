...
A flag is listed as consuming only when it consumes on every supported platform — over-listing drops an operand, under-listing only over-surfaces — which is why `sed -i` consumes the next argument only when it is empty.

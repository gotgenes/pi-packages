...
- `path` and `external_directory` each carry a **read/write axis** (ADR 0013 §3–§4, Refs #806), and the two directions are independent bits, not tiers — a `path_write` allow grants no read, a `path_read` deny floors no write.
  ...
  `parseUnresolvedAt` (`src/access-intent/bash/parser.ts`) is the single reader of `TSNode`'s `hasError`/`previousSibling`; do not hand-roll a sibling walk elsewhere, and do not widen it to the enclosing statement, which would forfeit the genuine `> out.txt` in `cat a > out.txt <> ~/rw.txt`.

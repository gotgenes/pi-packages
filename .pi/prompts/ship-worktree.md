...
1. `git rev-parse HEAD` to capture the full SHA; pass that exact value to `ci_find` (workflow `ci`).
...
- "Implemented in <sha> …" — the commit carrying the behavior, not the range's last commit.
  ...
  A SHA quoted from the plan or a stage note was written before the peer's rebase, so `git rev-parse` resolves it while it is unreachable.

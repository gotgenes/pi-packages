  ...
  resolve(scope: PersistentApprovalScope): PersistentApprovalTarget {
    if (scope === "global") {
      return {
        ...
        // An untrusted cwd must not be re-read on reload, or the write would
        // activate project config the operator never trusted.
...
export class PersistentApprovalService implements PersistentApprovalApi {
  ...
  /** Trust can change between the prompt and the write; resolve again now. */

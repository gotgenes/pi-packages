...
├── settings-policy.ts        Strict loader for the `piPermissionSystem` policy section in global/project Pi settings (`permission`, `subagentPermission`, and named `agents` overrides)
├── policy-loader.ts          PolicyLoader interface + FilePolicyLoader (settings/dedicated-config/frontmatter I/O, precedence, mtime caching); independently merges regular and subagent permission maps and marks a present-but-unloadable non-global scope `invalid` so composition can fail closed
├── scope-merge.ts            Cross-scope regular/subagent permission merge + origin-map bookkeeping
├── permission-manager.ts     Scope loading + rule composition + `check(intent)` (single resolution entry point); applies the subagent policy as a most-restrictive ceiling when the injected session predicate or explicit forwarded context identifies a subagent, floors composed rulesets `allow`→`ask` when a non-global scope is `invalid`, and appends a fail-closed notice to `getConfigIssues`. Constraint: stays string-based — must not import `AccessPath` (the ADR 0002 string boundary, lint-guarded by `no-restricted-imports`)
├── permission-gate.ts        Pure deny/ask/allow gate (injected IO)
├── permission-resolver.ts    `ScopedPermissionResolver` interface - the single `{ resolve(intent) }` role the gate factories / runner / pipeline depend on; `PermissionResolver` concrete class holds `ScopedPermissionManager` + `SessionRules`, owns `resolve(intent, context?)` (unwraps an `access-path` `AccessIntent` via `matchValues()` before calling `manager.check`; the optional concrete-only context marks forwarded requests as subagent-originated; the concrete class also accepts a pre-fixed `path-values` intent as a passthrough — the forwarded-serving wire's producer, #597 — while the gate-facing interface stays narrow to `AccessIntent`), raw `checkPermission` (`implements SkillPermissionChecker`, no session rules), `getToolPermission`, and `getConfigIssues`

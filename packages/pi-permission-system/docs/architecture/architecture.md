...
The roster shipped at 22 words with guards on `find`, `fd`, and `sort`.
Measured against the local review log by `scripts/measure-core-coverage.mjs` (802 bash asks, 228 recent), every unit head word is in the core for **22.8% of recent asks and 35.9% of all-time**.

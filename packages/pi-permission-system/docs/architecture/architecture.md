...
│       ├── redirect-analysis.ts Reads a `file_redirect` node well enough to consult the operator table: `redirectEffectForDestination(redirect, destination)` (the effect proved for one destination, `null` for a descriptor duplication) and `redirectProvesFileWrite(redirect)`, over one private operator lookup and one private descriptor-node set. Constraint: the token collector and the command enumerator ask different questions of the same read — what to attribute to a destination, and whether the statement writes at all — and two readers would drift. Note: tree-sitter-bash has no node for the read-write open `<>`, so what it proves depends on how the destination is spelled; two `it.fails` tests pin today's answer and #814 makes it unproven
...
Landed: measured by `scripts/measure-wrapper-transparency.mjs` (2026-08-27), wrapper-floored asks are 91 of 326 prompts across 2026-07 and 2026-08 (27.9%) and 43 of them are relieved — 13.2% of all prompts, 47.3% of floored ones.
...
The two functions share one `unwrapIndirection` walk and part company at the opaque layer instead; four asks in the log take exactly that shape.

...
Bounded delegation is operator config this package enforces at a checkpoint that downgrades an excluded-surface `allow` to `defer`, with secret-shaped `path` always excluded and `external_directory` excluded by default; the `allowAuthorizerOnExternalDirectory` config relaxes the `external_directory` exclusion (so a named link may grant `allow` on outside-CWD asks) while `path` stays excluded, and the model's provider/prompt/threshold live in the downstream extension's own config.

...
describe("PersistentPermissionWriter", () => {
  ...
  it("appends allow rules while preserving unrelated JSONC", () => {
    ...
    writer.write({
      path,
      expectedDir: root,
      grants: [{ surface: "bash", pattern: "gh workflow run *" }],
    });
...
`);
  ...
  it("records each grant on its own surface", () => {
    const writer = new PersistentPermissionWriter();
    ...
    writer.write({
      ...
      expectedDir: root,
      grants: [
        { surface: "external_directory:read", pattern: "/tmp/a" },
        { surface: "external_directory:write", pattern: "/tmp/b" },
        { surface: "external_directory:read", pattern: "/tmp/a" },
      ],
    });
    ...
    expect(JSON.parse(readFileSync(path, "utf8"))).toEqual({
      permission: {
        "external_directory:read": { "/tmp/a": "allow" },
        "external_directory:write": { "/tmp/b": "allow" },
      },
    });
  });

  it("rejects a blank pattern before touching the destination", () => {
    const writer = new PersistentPermissionWriter();
    ...
    expect(() =>
      writer.write({
        path,
        expectedDir: root,
        grants: [{ surface: "bash", pattern: "   " }],
      }),
    ).toThrow("non-empty permission pattern");
  ...
  it("creates the destination directory and writes multiple patterns atomically", () => {
    ...
    writer.write({
      ...
      expectedDir: nested,
      grants: [
        { surface: "external_directory", pattern: "/tmp/a" },
